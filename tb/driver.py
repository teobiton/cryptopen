...
from interface.bus.master import SimpleMaster
...
class Driver:
    ...
    async def write_block(self, block: int) -> None:
        ...
        for index, addr in enumerate(self.block_addrs):
            ...
            await self.master.write(value=data, address=addr)

    async def read_block(self) -> int:
        block: int = 0

        for index, addr in enumerate(self.block_addrs):
            data = await self.master.read(address=addr)
    ...
    async def enable(self, last_block=False) -> None:
        ...
        await self.master.write(value=value, address=self.ctrl_addr)

    async def disable(self) -> None:
        await self.master.write(value=0x0, address=self.ctrl_addr)

    async def reset(self) -> None:
        await self.master.write(value=0x2, address=self.ctrl_addr)

    async def read_hold(self) -> int:
        ctrlreg = await self.master.read(address=self.ctrl_addr)
        return ctrlreg & 0x8
